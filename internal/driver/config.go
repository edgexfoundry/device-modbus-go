...
func (info *ConnectionInfo) String() string {
	// Ignoring UnitID as we would be setting it for each request

	if info.Protocol == ProtocolTCP {
		return fmt.Sprintf("%s:%s:%d", info.Protocol, info.Address, info.Port)
	}
	return fmt.Sprintf("%s:%s:%d:%d:%d:%s", info.Protocol, info.Address, info.BaudRate, info.DataBits, info.StopBits, info.Parity)
}
